...
export class TelegramBotService {
    ...
    async start(@Ctx() ctx: Context) {
        ...
        if (user) {
            ...
            if (messages.length) {
                ...
                await ctx.reply('Кажется ты никому не нужен, даже чтобы послать тебя нахуй)');
            }
        }
    }
    ...
    async screwYou(@Ctx() ctx: Context) {
        ...
        await ctx.reply(`Привет ${user.userName}, кого ты хочешь послать нахуй сегодня?`);
    }
    ...
    async onText(@Ctx() ctx: Context) {
        if (this._isScrewYouFlowActive) {
            ...
            try {
                ...
                this._generateScrewYouMessage(user.id, newUser.id);
    ...
    private async _getUser(ctx: Context) {
        try {
            return await this.userService.getUser(ctx.message.from.id);
        } catch (error) {
            return await this._handleNewUser(ctx);
        }
    }
